...
export function AdminClubs() {
  ...
  return (
    <div className="space-y-4">
      ...
      {data && (
        <p className="text-slate-500 text-xs">{data.total} clubs · page {data.page} of {totalPages}</p>
      )}
            ...
            {data?.items.map(club => (
              <tr key={club.id} className="hover:bg-white/[0.02]">
                ...
                <td className="px-3 py-2">
                  <EditableCell value={club.isAliased ? club.name : ''} onSave={v => handleSave(club, 'name', v)} placeholder="Click to add alias" />
                </td>
                <td className="px-3 py-2">
                  <EditableCell value={club.shortName ?? ''} onSave={v => handleSave(club, 'shortName', v)} placeholder="—" />
                </td>
                <td className="px-3 py-2">
                  <EditableCell value={club.city ?? ''} onSave={v => handleSave(club, 'city', v)} placeholder="—" />
                </td>
                <td className="px-3 py-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${club.isAliased ? 'bg-game-neon/10 text-game-neon' : 'bg-game-fire/10 text-game-fire'}`}>
